...
public class CxService implements CxClient {
    ...
    */
    ...
    @Override
    public Integer createPDFScanReport(Integer scanId) {
        String strJSON = "{'reportType':'PDF', 'scanId':%d}";
        strJSON = String.format(strJSON, scanId);
    ...
    @Override
    public byte[] downLoadPDFReport(ScanResults reportId) {
    ...
    public ScanResults getPDFReportId(Integer scanId, FilterConfiguration filter) throws CheckmarxException{
        Integer reportId = createPDFScanReport(scanId);
        try {
            waitForReportCreateOrFail(reportId);
            Thread.sleep(1000);
        } catch (InterruptedException e) {
